/*
 * drivers/input/keyboard/sunxi-cir.c
 *
...
#define IR_RXFILT_VAL		(5)	//  Filter Threshold = 5*42.7 = ~213.5us < 274us

#define IR_SEPARATOR_MIN	(5)	//  274/42.7=6.416
#define IR_SEPARATOR_MAX	(13)

#define IR_LEN0_MIN		(9)	//  780/42.7=18.267
#define IR_LEN0_MAX		(18)

#define IR_LEN1_MIN		(33)	//  1836/42.7=42.997
#define IR_LEN1_MAX		(42)
...
static void ir_reg_cfg(void)
{
	unsigned long tmp = 0;
	/* Enable IR Mode */
	tmp = 0x3 << 4; //CIR mode for Remote control or wireless keyboard
...
/*	printk("dcnt = %d\n", (int)dcnt);
	...
	{
		if(buf[i] & 0x80)
			printk(" 1 %d,", (int)(buf[i] & 0x7f));
		else
			printk(" 0 %d,", (int)(buf[i] & 0x7f));
	...
	while(i<dcnt)
	{
		...
		// The Impulse is too long we have reached the end of buffer
		if(val & 0x80)
			return IR_ERROR_CODE; // Invalid Code
		
		if( len > IR_SEPARATOR_MAX || len < IR_SEPARATOR_MIN )
		{	// The separator impulse is out of tolerance, continue from the beginning
			code = 0;
			bitcnt = 0;
			i++;
			continue;
		...
		// check for last bit and the end of trasmission
		if( len > IR_LEN1_MAX && bitcnt == 16 )
		{
//			printk("Scanned code  %d\n", (int)(code));
			return code;
		}
		...
		if( len < IR_LEN0_MIN || len > IR_LEN1_MAX || ( len > IR_LEN0_MAX && len < IR_LEN1_MIN ) )
		{	// The bit impulse is out of tolerance, continue from the beginning
			code = 0;
			bitcnt = 0;
			i++;
			continue;
...
//				ir_cnt = 0;
		...
		//	if (ir_cnt <= 2) 
			{
				if (code_valid){
//					ir_code = (code & 0xff) >> 2; /* update saved code with a new valid code */
#ifdef DEBUG_IR_LEVEL0
				printk("IR RAW CODE : %lu\n", (ir_code /*>> 16*/) & 0xff);
#endif
//				input_report_key(ir_dev, ir_keycodes[(ir_code /*>> 16*/) & 0xff], ir_cnt );
				if(ir_cnt<=2)
					input_event(ir_dev, EV_KEY ,ir_keycodes[(ir_code /*>> 16*/) & 0xff], ir_cnt );
				else
					input_event(ir_dev, EV_KEY ,ir_keycodes[(ir_code /*>> 16*/) & 0xff], 2 );

//				printk("input_report_key ir_cnt=%d\n",(int)ir_cnt);
#ifdef DEBUG_IR_LEVEL0
...
static void ir_timer_handle(unsigned long arg)
{
	...
	input_sync(ir_dev);
#ifdef DEBUG_IR_LEVEL1
	printk("IR KEY TIMER OUT UP ir_cnt=%d\n",(int)ir_cnt);
#endif
	ir_cnt = 0;
